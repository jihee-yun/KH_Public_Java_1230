...
public class Car { // 클래스 이름은 대문자로 시작한다
    ...
    // 생성자는 클래스가 객체로 만들어질 때 호출되고, 생성자를 만들지 않으면 보이지 않는 생성자가 호출 된다
    // 생성자는 여러개 만들어 필요에 따라 적절한 생성자를 호출할 수 있음 (생성자 오버로딩)
    // 생성자 매개변수는 클래스를 객체로 만드는 위치에서 전달
    // 생성자의 목적은 클래스가 객체로 만들어질 때 해당 인스턴스 필드를 초기화 하는 것이 주 목적
    Car() {
        productName = "이름없는 차";
        this.color = "색상도 없음";
        maxSpeed = 0;
