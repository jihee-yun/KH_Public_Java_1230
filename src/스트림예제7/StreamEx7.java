...
// 최종 연산 시 모든 요소를 소모한 해당 스트림은 더이상 사용할 수 없다
// 요소의 출력 : forEach()
// 요소의 소모 : reduce() : 첫번째와 두번째 요소를 가지고 연산을 수행한 뒤, 그 결과를 다시 세번째 요소와 비교
...
public class StreamEx7 {
    public static void main(String[] args) {
        ...
        OptionalInt rst1 = stream3.sorted().findFirst(); // 정렬이후 첫번째 값을 가져 옴
        System.out.println(rst1.getAsInt());
        OptionalInt rst2 = stream4.sorted().findAny(); // 정렬이후 첫번째 값 (병렬 처리 시 결과 달라질 수 있음)
        System.out.println(rst2.getAsInt());

        // 요소의 검사
        // - anyMatch() : 해당 스트림의 일부 요소가 특정 조건을 만족할 경우 true
        // - allMatch() : 해당 스트림의 모든 요소가 특정 조건을 만족할 경우 true
        // - noneMatch() : 해당 스트림의 모든 요소가 특정 조건을 불만족할 경우 true
