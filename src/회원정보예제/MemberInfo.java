...
public class MemberInfo {
    ...
    String name;
    int age; // 나이는 1 ~ 199까를 유효값으로 간주
    char gender; // 'M' 과 'm' 은 남성, 'F' 와 'f'는 여성으로 간주하고 출력은 "남성", "여성"으로
    int jobs; // 정수로 입력(1.학생, 2.회사원, 3.주부, 4.무직)
    Scanner sc = new Scanner(System.in); // 키보드 입력을 위해서 스캐너 생성
    // 메소드 추가하기 (클래스 외부에 존재하면 함수라고 부르는데 자바는 메소드가 클래스 외부 존재 불가)
    ...
    public void setAge() {
        while(true) {
            ...
            age = sc.nextInt();
            if (age > 0 && age < 200) return; // 메소드를 호출한 곳으로 되돌아감. 즉, 메소드 수행 종료
    ...
    public void setGender() {
        while (true) {
            ...
            gender = sc.next().charAt(0); // 문자열에서 첫번째 문자 추출
            switch (gender) { // 조건식
                case 'M':
                case 'm':
                case 'F':
    ...
    public void setJobs() {
        while(true) {
            ...
            System.out.println("직업을 잘못 입력 하였습니다.");
        }
    }
    ...
    public void viewInfo() {
        ...
        String[] jobsStr = {"", "학생", "회사원","주부","무직"}; // 배열 index 0부터 시작
