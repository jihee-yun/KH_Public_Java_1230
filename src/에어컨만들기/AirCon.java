...
public class AirCon {
    ...
    public AirCon() { // 생성자 : 클래스가 객체화될 때 자동으로 호출, 인스턴스 필드 초기화가 주 목적
        final int[] monthTemp = {-5, 3, 8, 15, 20, 26, 32, 30, 24, 16, 8, 4}; // 읽기 위한 용도
        Calendar now = Calendar.getInstance();
        int month = now.get(Calendar.MONTH); // 현재 달의 정보를 가져오는데 정보가 배열의 인덱스와 같음
        currTemp = monthTemp[month];
        isPower = false; // 전원에 대한 인스턴스 필드는 OFF 가 default
        setTemp = 18;
        isCooler = false;
        isHeater = false;
        windStep = 1; //
    ...
    public void airConInfo() {
        String onOffStr = (isPower) ? "ON" : "OFF"; // isPower 가 참이면 ON, 거짓이면 OFF
