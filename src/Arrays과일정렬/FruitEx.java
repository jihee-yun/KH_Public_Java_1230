...
// 정렬은 기본적으로 오름차순
// 임의의 10개 과일 이름 배열 생성, 길이순으로 정렬하고 만약 길이가 같으면 사전순으로 정렬
// 문자열 길이를 비교하기 : length()
...
//        for(int e : arr) System.out.print(e +  " ");
        ...
        String[] fruit = {"Apple", "Banana", "Mango", "Watermelon", "Melon", "Peach"
        ,"Orange", "Cherry", "Kiwi", "Plum"};
        Arrays.sort(fruit, new Comparator<String>() {
            @Override
            public int compare(String o1, String o2) {
                if(o1.length() > o2.length()) return 1; // 첫번째가 두번째보다 길기 때문에 정렬에 해당
                else {
                    if(o1.length() == o2.length()) {
                        return o1.compareTo(o2); // 첫번째보다 두번째가 사전 순서상 앞이면 양수
                    }
                    return -1; // 정렬하지 않는 조건
                }
